...
struct GeneratedContentView: View {
    ...
    @State private var hasAutoGeneratedInitialVisuals = false
        ...
        NavigationStack {
            ...
            .sheet(item: $generatedVisualPreview) { preview in
                GeneratedStyleVisualView(
                    ...
                    useButtonTitle: preview.target.generatedButtonTitle
                ) {
            ...
            .alert(
                ...
                isPresented: Binding(
                    get: { visualErrorMessage != nil },
                    set: { if !$0 { visualErrorMessage = nil } }
                ),
                actions: {
                ...
                message: {
                    Text(visualErrorMessage ?? "Попробуйте ещё раз.")
                }
            )
            .overlay {
                    ...
                    ZStack {
                        ...
                        KadroCard {
                            VStack(spacing: 12) {
                                ProgressView()
                                    .tint(.kadroCharcoal)
                                Text(visualLoadingTitle)
                                    .font(.kadroTitle3)
                                    .foregroundColor(.kadroCharcoal)
                                Text(visualLoadingSubtitle)
                                    .font(.kadroCallout)
            ...
            .task {
                await autoGenerateVisualsIfNeeded()
            }
        }
    }
                ...
                if let selectedStylePack {
                    ...
                    HStack(spacing: 10) {
                        actionButton(title: project.generatedCoverImageData == nil ? "Создать cover" : "Перегенерировать cover") {
                            Task { await generateCoverVisual() }
                        }
                    ...
                    Text("Style pack не выбран. Выберите стиль на этапе создания контента.")
                        .font(.kadroCallout)
                        .foregroundColor(.kadroWarmGray)
                }
            }
                    ...
                    KadroCard {
                        VStack(alignment: .leading, spacing: 10) {
                            ...
                            actionButton(title: slide.generatedImageData == nil ? "Создать visual для слайда" : "Перегенерировать visual") {
                                Task { await generateSlideVisual(slide) }
                            }
                        }
                    }
                }
    ...
    @MainActor
    private func autoGenerateVisualsIfNeeded() async {
        guard !hasAutoGeneratedInitialVisuals else { return }
        hasAutoGeneratedInitialVisuals = true
        guard selectedStylePack != nil, selectedStylePackReferenceCount > 0 else { return }
        
        if project.generatedCoverImageData == nil {
            await generateCoverVisual(autoMode: true)
        }
        
        if project.type == .carousel && slides.contains(where: { $0.generatedImageData == nil }) {
            await generateAllSlideVisuals(autoMode: true)
        }
    }
    
    @MainActor
    private func generateCoverVisual(autoMode: Bool = false) async {
        guard !isGeneratingVisual else { return }
        isGeneratingVisual = true
        visualLoadingTitle = autoMode ? "Готовим visual preview" : "Генерируем cover"
        visualLoadingSubtitle = autoMode ? "Создаём первый visual preview на основе выбранного style pack." : "Собираем reference-guided prompt для visual cover на основе style pack и ваших референсов."
        
        do {
            let response = try await styleImageService.generateCoverVisual(project: project, brandProfile: brandProfile)
            if autoMode {
                try persistGeneratedVisual(response, target: .cover)
            } else {
    ...
    @MainActor
    private func generateSlideVisual(_ slide: CarouselSlide) async {
        guard !isGeneratingVisual else { return }
        isGeneratingVisual = true
    ...
    @MainActor
    private func generateAllSlideVisuals(autoMode: Bool = false) async {
        guard !isGeneratingVisual else { return }
        isGeneratingVisual = true
        
        for (index, slide) in slides.enumerated() {
            if !autoMode || slide.generatedImageData == nil {
                visualLoadingTitle = autoMode ? "Готовим visuals карусели" : "Генерируем visuals для карусели"
