...
enum SupabaseConfig {
    ...
    static let transcribeAudioFunctionName = "transcribe-audio"
    
    static var generateContentURL: URL {
        URL(string: projectURLString + "/functions/v1/" + generateContentFunctionName)!
    }
    
    static var transcribeAudioURL: URL {
        URL(string: projectURLString + "/functions/v1/" + transcribeAudioFunctionName)!
    }
}
